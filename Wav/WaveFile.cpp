...
HRESULT CWaveFile::Open(LPWSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags)
{
	...
	if (m_dwFlags == WAVEFILE_READ)
	{
		...
		//MessageBox(NULL, L"THIS WAY.", L"T", MB_OK);
	}
	else
	{
		m_hmmio = mmioOpen(strFileName, NULL, MMIO_ALLOCBUF |
			MMIO_READWRITE |
			MMIO_CREATE);
		MessageBox(NULL, strFileName, L"T", MB_OK);
		if (NULL == m_hmmio) {
			MessageBox(NULL, L"WAV文件不存在.", L"T", MB_OK);
			return -8;
		}
...
HRESULT CWaveFile::ReadMMIO()
{
	...
	if ((0 != mmioDescend(m_hmmio, &m_ckRiff, NULL, 0)))
		return 1;// DXTRACE_ERR(L"mmioDescend", E_FAIL);
